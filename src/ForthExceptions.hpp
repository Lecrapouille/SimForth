...
//! Exceptions are derived from the project common ancestor SimForth::Exception
...
public:
  ForthDefinition(std::string const& word)
    : ForthException(40)
  {
    m_msg = "Exception from SimForthForth: word '" + word + "' contains more than 2^16 words.";
  }
};
...
public:
  MalformedForthWord(std::string const& word)
    : ForthException(39)
  {
    m_msg = "Exception from SimForthForth: word '" + word + "' is mal formed. It shall contain 1 ... 31 characters.";
  }
};
...
public:
  UnknownForthPrimitive(const Cell16 badToken, std::string const& funcName)
    : ForthException(36)
  {
    m_msg = "Exception from SimForthForth: try to execute an unknown primitive "
