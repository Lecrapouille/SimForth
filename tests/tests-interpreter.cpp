...
TEST(CheckInterpreter, PassingParameters)
{
    ...
    ASSERT_EQ(forth.dataStack().pick(1).integer(), 42);
    ASSERT_EQ(DS.depth(), 2);
    ASSERT_EQ(DS.pick(0).integer(), 55);
    ASSERT_EQ(DS.pick(1).integer(), 42);

    // Replace the data on the top of the stack
    DDROP();
    DPUSHI(43);
    ASSERT_EQ(forth.dataStack().depth(), 2);
    ASSERT_EQ(forth.dataStack().pick(0).integer(), 43);
    ASSERT_EQ(forth.dataStack().pick(1).integer(), 42);
    ...
    ASSERT_EQ(forth.interpretString("FOO"), true);
    ASSERT_EQ(forth.dataStack().depth(), 1);
    ASSERT_EQ(forth.dataStack().pick(0).integer(), 85);

    DataStack const& cDS = forth.dataStack();
    ASSERT_EQ(cDS.depth(), 1);
    ASSERT_EQ(cDS.pick(0).integer(), 85);
}
...
TEST(CheckInterpreter, Verbose)
{
    ...
    std::stringstream buffer;
    std::streambuf* old = std::cout.rdbuf(buffer.rdbuf());
    ASSERT_EQ(forth.interpretString("TRACES.ON : FOO 1 + - ; 2.3 4 FOO"), true);
    std::cout.rdbuf(old);
    ...
    EXPECT_THAT(buffer.str().c_str(), HasSubstr("Compile word +"));
    EXPECT_THAT(buffer.str().c_str(), HasSubstr("Compile word -"));
    ...
    ASSERT_EQ(forth.dataStack().depth(), 1);
    EXPECT_NEAR(forth.dataStack().pick(0).real(), -2.7, 0.000001);
}
...
TEST(CheckInterpreter, IntegerOverflow)
{
    ...
    ASSERT_EQ(forth.dataStack().depth(), 1);
    ASSERT_EQ(forth.dataStack().pick(0).isInteger(), true);
    ...
    ASSERT_STREQ(buffer.str().c_str(), "");
    ASSERT_EQ(forth.dataStack().depth(), 1);
    ASSERT_EQ(forth.dataStack().pick(0).isReal(), true);
    ASSERT_EQ(forth.dataStack().pop().real(), 92233720368547758080.000);
